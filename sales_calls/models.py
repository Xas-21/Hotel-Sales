...
class SalesCall(models.Model):
    ...
    account = models.ForeignKey(Account,
                                ...
                                related_name='sales_calls')
    visit_date = models.DateField(db_index=True)
    city = models.CharField(max_length=100)
    address = models.TextField(blank=True)

    # Meeting details
    meeting_subject = models.CharField(
        max_length=50,
        choices=MEETING_SUBJECT)  # Increased length for longer choice labels
    business_potential = models.CharField(max_length=10,
                                          choices=BUSINESS_POTENTIAL,
                                          default='Unknown')

    # Detailed tracking
    next_steps = models.TextField(
        help_text="Planned next steps from this meeting")
    detailed_notes = models.TextField(
        help_text="Detailed notes from the meeting")
